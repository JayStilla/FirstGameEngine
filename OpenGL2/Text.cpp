...
{
    float color4f[4];//giving color values
...
void OpenGLText::drawString( int x, int y, const char * text, int nbLines, float * color4f)
{
    ...
    const char* c = text;
    Vertex v;
    v.color[0] = color4f[0];//assigning the color values for the font
    v.color[1] = color4f[1];
    v.color[2] = color4f[2];
    v.color[3] = color4f[3];
    ...
    while (*c != '\0')
    {
        ...
        else
        {
            GlyphInfo &g = glyphInfos->glyphs[*c];
            int pX = lPosX + g.pix.offX;
            int pY = lPosY - g.pix.height - g.pix.offY;//edits the position of the font on the screen
            beginStrip();
